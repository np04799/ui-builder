...
export default function BannerMorphProps({ element }: Props) {
  const updateElement = useBuilderStore((s) => s.updateElement)
  if (element.content.type !== 'banner-morph') return null
    ...
    <React.Fragment>
      <PropGroup label="Content">
        <PropRow label="Badge" stack>
          <TextInput value={content.badge} onChange={(v) => patch({ badge: v })} placeholder="Tag label" />
        </PropRow>
