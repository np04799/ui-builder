...
const CUTOUT_OPTIONS = [
  { label: 'Thin (80%)', value: '80' },
  { label: 'Medium (65%)', value: '65' },
  { label: 'Thick (50%)', value: '50' },
]

interface Props { element: ElementNode }

export default function DonutChartProps({ element }: Props) {
  const updateElement = useBuilderStore((s) => s.updateElement)
  if (element.content.type !== 'chart-donut') return null
  ...
  return (
    <>
      <PropGroup label="Chart Settings">
        ...
        <PropRow label="Hole size">
          <SelectInput value={String(content.cutout ?? 65)} options={CUTOUT_OPTIONS} onChange={(v) => patch({ cutout: parseInt(v) })} />
        </PropRow>
