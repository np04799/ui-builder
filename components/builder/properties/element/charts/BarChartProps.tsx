...
export default function BarChartProps({ element }: Props) {
  const updateElement = useBuilderStore((s) => s.updateElement)
  if (element.content.type !== 'chart-bar') return null
    ...
    <>
      <PropGroup label="Chart Settings">
        ...
        <PropRow label="Orientation">
          <SelectInput value={content.orientation ?? 'vertical'} options={ORIENTATION_OPTIONS} onChange={(v) => patch({ orientation: v as 'vertical' | 'horizontal' })} />
        </PropRow>
