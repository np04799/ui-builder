...
export default function DashboardPreviewPage() {
  ...
  if (!hydrated || widgets.length === 0) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', fontFamily: 'sans-serif', color: '#888', flexDirection: 'column', gap: 12 }}>
        ...
        <p style={{ margin: 0, fontSize: '0.9rem' }}>{!hydrated ? 'Loading dashboard…' : 'No dashboard widgets to preview.'}</p>
        {hydrated && <p style={{ margin: 0, fontSize: '0.8rem' }}>Add widgets in BuilderPro and click Preview again.</p>}
      </div>
    )
  }

  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f8fafc', padding: '16px 20px 80px', fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, paddingBottom: 12, borderBottom: '1px solid #e2e8f0' }}>
        ...
        <span style={{ fontSize: '0.75rem', color: '#94a3b8', background: '#f1f5f9', padding: '3px 10px', borderRadius: 20, border: '1px solid #e2e8f0' }}>
      ...
      >
        {widgets.map((widget) => (
          <div
            key={widget.id}
            style={{ background: '#fff', borderRadius: 10, border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', display: 'flex', flexDirection: 'column' }}
          >
            {widget.style?.showTitle !== false && widget.type !== 'filter-bar' && (
              <div style={{ padding: '8px 12px 6px', fontSize: '0.8125rem', fontWeight: 600, color: widget.style?.titleColor ?? '#374151', borderBottom: '1px solid #f1f5f9', flexShrink: 0 }}>
                {widget.title}
              </div>
            )}
